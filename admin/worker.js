...
async function api(request, env) {
  ...
  if (request.method === 'GET' && url.pathname === '/api/oshxona/names-lite') {
    ...
    const all = JSON.parse(match[1]); const q = (url.searchParams.get('q') || '').trim().toLowerCase(); const genders = (url.searchParams.get('gender') || '').split(',').map((value) => value.trim()).filter(Boolean); const originsFilter = (url.searchParams.get('origin') || '').split(',').map((value) => value.trim().toLowerCase()).filter(Boolean); const lettersFilter = (url.searchParams.get('letter') || '').trim().toLowerCase(); const statuses = (url.searchParams.get('status') || '').split(',').map((value) => value.trim()).filter(Boolean); const offset = Math.max(0, Number(url.searchParams.get('offset') || 0)); const limit = Math.min(50, Math.max(1, Number(url.searchParams.get('limit') || 50))); const filtered = all.filter((item) => { const name = String(item.l || ''); const variants = String(item.k || ''); const itemOrigin = String(item.lang || ''); return (!q || name.toLowerCase().includes(q) || variants.toLowerCase().includes(q)) && (!genders.length || genders.includes(item.g)) && (!originsFilter.length || originsFilter.includes(itemOrigin.toLowerCase())) && (!lettersFilter || name.trim().charAt(0).toLowerCase() === lettersFilter) && (!statuses.length || statuses.includes('published')); }); const page = filtered.slice(offset, offset + limit); const rows = page.map((item) => ({ id: String(item.id), slug: String(item.l || '').toLowerCase().replace(/[^a-z0-9а-яё']+/gi, '-').replace(/^-|-$/g, ''), name: item.l, gender: item.g, origin: item.lang || '', meaning: item.m || '', variants: item.k || '', status: 'published', seo_description: `${item.l} ismining ma'nosi, kelib chiqishi va yozilish variantlari.`, updated_at: '' })); const origins = [...new Set(all.map((item) => String(item.lang || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b)); const letters = [...new Set(all.map((item) => String(item.l || '').trim().charAt(0).toUpperCase()).filter(Boolean))].sort((a, b) => a.localeCompare(b)); return json({ user: email, rows, total: filtered.length, offset, limit, hasMore: offset + rows.length < filtered.length, origins, letters, counts: { total: all.length, published: all.length, review: 0, draft: 0 }, fallback: true });
  }
